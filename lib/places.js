...
function isAllowedCategory(categories) {
  ...
  // Judge by the PRIMARY category only. HERE attaches many incidental tags, and
  // a legit tradie often ALSO carries a 600-/800- (leisure/shopping) tag that
  // must not kill it (real painters were being dropped this way). Drop only
  // when the business's PRIMARY category is itself an excluded type
  // (real estate, food, retail, etc.). The name filters (EXCLUDE_NAME_PATTERNS
  // + strict render matching) catch the rest as belt-and-braces.
  const primary = categories.find((c) => c?.primary) || categories[0];
  const id = primary?.id || '';
  return !EXCLUDE_CATEGORY_PREFIXES.some((p) => id === p || id.startsWith(p));
}
 ...
 * }>>}
 */
// Global concurrency limiter for HERE calls. A report's tradie lookup wants to
// fire ~45 discover calls at once, which bursts past HERE's free-tier rate
// limit (429). Cap simultaneous requests to a few so calls succeed instead of
// being dropped. Combined with the 429 retry below, this keeps tradie buckets
// reliably populated.
...
const hereLimit = makeLimiter(3);

async function discoverNearby(query, coords, apiKey) {
  return hereLimit(() => _discoverNearbyImpl(query, coords, apiKey));
}

async function _discoverNearbyImpl(query, coords, apiKey) {
  const url =
    `https://discover.search.hereapi.com/v1/discover` +
 ...
 */
async function findTradiesForTrade(tradeKey, geo, apiKey, TRADES, strictMatch) {
  ...
  return unique
    ...
    .filter((r) => isRepairTradie(r.business_name))
    .filter((r) => isAllowedCategory(r.categories))
    // STRICT: the business name must genuinely identify THIS trade — applied
    // BEFORE the top-2 slice so closer wrong-named businesses (fencing/
    // demolition under "landscaper") don't crowd out a real (slightly further)
    // match. Stored bucket = the 2 closest GENUINE matches.
    .filter((r) => !strictMatch || strictMatch(r.business_name, tradeKey))
    .sort((a, b) => a.distance_m - b.distance_m)
...
export async function findTradiesForAnalysis(analysis) {
  ...
  const { TRADES, topTradesForDefect, tradeByKey, strictTradieMatch } = await import('./trades.js');
  ...
  if (tradeKeys.size === 0) return {};

  // Geocode once — share coords across every HERE lookup below.
  const geo = await geocode(address, apiKey);
  if (!geo) return {};

  // Per-trade-key lookups only. The old broad trade_category buckets
  // (building/plumbing/damp/etc.) are retired — they weren't strict-filtered
  // and the render no longer uses them (it keys off the specific inferred
  // trades). Dropping them removes dead weight + extra HERE calls.
  const perTrade = await Promise.all(
    Array.from(tradeKeys).map(async (key) => [
      key,
      await findTradiesForTrade(key, geo, apiKey, TRADES, strictTradieMatch),
    ])
  );
