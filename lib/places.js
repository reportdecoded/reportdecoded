...
export async function findTradiesForAnalysis(analysis) {
  ...
  const { TRADES, topTradesForDefect } = await import('./trades.js');

  // NEW: per-defect inferred trade keys (carpenter, concreter, etc).
  // We collect the union across all defects — primary AND secondary —
  // so trade-interface defects get tradies from BOTH specialties.
  const tradeKeys = new Set();
  for (const d of defects) {
    const trades = topTradesForDefect(d);
    for (const t of trades) tradeKeys.add(t.key);
  }
