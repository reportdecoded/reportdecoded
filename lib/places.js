...
const DISCOVER_FETCH_LIMIT = 20; // ask for plenty; we'll filter by distance
const MAX_DISTANCE_M = 50_000;   // 50 km cap — beyond that it's not a "local tradie"
...
export async function findTradies({ tradeCategory, address, limit = MAX_RESULTS_PER_CATEGORY, geo }) {
  ...
  return items
    ...
    .filter((it) => typeof it.distance === 'number' && it.distance <= MAX_DISTANCE_M)
