...
export function trackPurchase({ value, currency = 'AUD', contentName, transactionId } = {}) {
  safeFbq(
    'track',
    'Purchase',
    { value, currency, content_name: contentName },
    // eventID lets Meta dedupe if a server-side Conversions API event is
    // ever added for the same purchase. Harmless when absent.
    transactionId ? { eventID: transactionId } : undefined
  );
  safeGtag('event', 'purchase', {
    // transaction_id is how GA4 dedupes a purchase if the success page is
    // reloaded/back-buttoned — the Stripe session id is the natural key.
