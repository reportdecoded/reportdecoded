...
function reportReadyHtml({
  ...
  reportType,               // 'pre_purchase' | 'new_build_handover' — drives label vocab
}) {
  const isHandover = reportType === 'new_build_handover';

  // Handover reports need different vocabulary in the email — "Negotiate"
  // off the contract makes no sense once the buyer has signed. They need
  // to "Rectify" defects before sign-off / final payment.
  const v0 = VERDICT_META[verdict] || { color: '#6B7280', bg: '#F0EDE8', label: verdict || 'COMPLETE', tone: '' };
  const v = isHandover
    ? {
        ...v0,
        label:
          ...
          : verdict === 'WALK_AWAY' ? 'ESCALATE'
          : v0.label,
        tone:
          verdict === 'NEGOTIATE' ? 'Defects to rectify before sign-off — here\'s the list.'
          : verdict === 'PROCEED' ? 'Clean handover — ready to sign off.'
          : verdict === 'WALK_AWAY' ? 'Major contract breach — escalate to the VBA.'
          : v0.tone,
      }
    : v0;
  ...
  const negotiationBlock = negFormatted
    ? `<div style="background:#FEF3E8;border:1px solid #F4C9A0;border-radius:10px;padding:16px 18px;margin:20px 0">
        <div style="font-size:11px;color:#92400E;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;margin-bottom:4px">${isHandover ? 'Rectification value' : 'Suggested negotiation'}</div>
        <div style="font-family:Georgia,serif;font-size:30px;color:#0A1628;letter-spacing:-0.5px">${negFormatted}${isHandover ? ' outstanding' : ' off'}</div>
...
export async function sendReportReadyEmail({
  ...
  isInvestor = false,   // true when purchase_intent === 'investment'
  reportType,           // 'pre_purchase' | 'new_build_handover'
}) {
  const payload = {
    from: FROM,
    to,
...
export async function sendRefundNotificationEmail({ to, reason, refundIssued = false }) {
  // Only claim a refund was issued if Stripe actually confirmed it.
  // Previously this always said "We've issued a full refund" regardless
  // of whether a Stripe API call was made — which was a lie.
  const refundLine = refundIssued
    ? `<p><strong>A full refund has been issued to your card.</strong> It should appear within 5 business days.</p>`
    : `<p>You have <strong>not been charged</strong> for this analysis attempt. If you see a pending charge, it will drop off automatically — or reply to this email and we'll confirm.</p>`;

  return getResend().emails.send({
    from: FROM,
    to,
    subject: refundIssued
      ? 'Report Decoded — we couldn\'t analyse your report (refund issued)'
      : 'Report Decoded — analysis failed, you have not been charged',
    html: `<!doctype html><html><body style="font-family:Arial,sans-serif;padding:32px;max-width:560px;margin:0 auto">
      ...
      ${refundLine}
      <p>If you think this was a mistake, or if you need anything at all, reply to this email and we'll sort it out manually.</p>
      <p>— Report Decoded</p>
    </body></html>`,
  });
}
...
// ── Sample report email (email capture) ──────────────────────────────
// Fires when a visitor submits their email on the homepage "Not ready?"
// form. Sends the Yarraville sample link so they can see the product
// before committing $39. Kept very short — one CTA, no friction.

export async function sendSampleReportEmail({ to }) {
  const base = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.reportdecoded.com.au';
  const sampleUrl = `${base}/results?reportId=f3ef0ce1-5443-4e91-a420-5e8bf7d8713d&sample=1`;

  return getResend().emails.send({
    from: FROM,
    to,
    subject: 'Your free sample report from Report Decoded',
    html: `<!doctype html>
<html><body style="font-family:'DM Sans',Arial,sans-serif;background:#F7F3EE;margin:0;padding:32px 16px;color:#1C1917">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:32px 30px;border:1px solid #E5E0D8">

    <div style="margin-bottom:20px">
      <img src="${base}/logo-light.png" alt="Report Decoded" width="180" style="display:block;max-width:180px;height:auto;filter:brightness(0)" />
    </div>

    <h1 style="font-family:Georgia,serif;font-size:24px;margin:0 0 12px;color:#0A1628;letter-spacing:-0.3px">
      Here's what your report could look like.
    </h1>

    <p style="font-size:14px;line-height:1.65;color:#374151;margin:0 0 20px">
      This is a real analysis of a Yarraville property — 85-page inspector PDF turned into a plain-English verdict,
      defect-by-defect repair cost estimates, matched tradies, and a ready-to-send negotiation letter. Takes under 2 minutes on your own report.
    </p>

    <div style="text-align:center;margin:24px 0">
      <a href="${sampleUrl}"
         style="display:inline-block;background:#C97A3A;color:#fff;text-decoration:none;padding:13px 28px;border-radius:10px;font-weight:600;font-size:15px">
        View the sample report →
      </a>
    </div>

    <p style="font-size:13px;line-height:1.55;color:#6B7280;margin:24px 0 0">
      When you're ready, it's $39 for a single analysis — no subscription. Upload at
      <a href="${base}" style="color:#C97A3A;font-weight:600">reportdecoded.com.au</a>.<br/><br/>
      Morgan · Founder, Report Decoded
    </p>
  </div>
</body></html>`,
  });
...
// ── Buyer Day 1 follow-up ─────────────────────────────────────────────
// Fires ~24h after report completion. Tone: warm check-in, not pushy.
// Goal: close the loop on whether the negotiation landed, surface the
// negotiation letter article as useful context, and stay top-of-mind
// before they move to the next property search.

export async function sendBuyerDay1FollowupEmail({ to, propertyAddress, reportId }) {
  const base = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.reportdecoded.com.au';
  const reportUrl = `${base}/results?reportId=${reportId}`;
  const negArticleUrl = `${base}/resources/building-inspection-negotiation-letter-template`;
  const addressLine = propertyAddress
    ? `for <strong style="color:#0A1628">${propertyAddress}</strong>`
    : 'on your recent property';

  return getResend().emails.send({
    from: FROM,
    to,
    subject: 'How did the negotiation go?',
    html: `<!doctype html>
<html><body style="font-family:'DM Sans',Arial,sans-serif;background:#F7F3EE;margin:0;padding:32px 16px;color:#1C1917">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:32px 30px;border:1px solid #E5E0D8">
    ...
    <h1 style="font-family:Georgia,serif;font-size:23px;margin:0 0 12px;color:#0A1628;letter-spacing:-0.3px">
      Quick check-in
    </h1>

    <p style="font-size:14px;line-height:1.65;color:#374151;margin:0 0 16px">
      You ran a report ${addressLine} yesterday. Just checking in — did the negotiation language help?
      If you used the suggested figure, I'd genuinely love to know how it landed.
    </p>

    <p style="font-size:14px;line-height:1.65;color:#374151;margin:0 0 20px">
      If you're still in the negotiation phase, two things that might help:
    </p>

    <ul style="font-size:14px;line-height:1.8;color:#374151;padding-left:20px;margin:0 0 20px">
      <li>
        <a href="${reportUrl}" style="color:#C97A3A;font-weight:600">Your report</a> — re-share the negotiation letter to your agent any time.
      </li>
      <li>
        <a href="${negArticleUrl}" style="color:#C97A3A;font-weight:600">How to write a negotiation letter</a> — a plain-English guide to what to say, and what not to.
      </li>
    </ul>

    <p style="font-size:13px;line-height:1.55;color:#6B7280;margin:24px 0 0">
      Reply to this email if anything's unclear — it goes straight to me.<br/><br/>
      Morgan · Founder, Report Decoded<br/>
      <a href="mailto:info@reportdecoded.com.au" style="color:#C97A3A">info@reportdecoded.com.au</a>
    </p>
  </div>
</body></html>`,
...
// ── Buyer Day 7 upsell ────────────────────────────────────────────────
// Fires ~7 days after report completion. By now the buyer has either
// settled, walked away, or started a new property search. Goal: surface
// the 3-pack value for active buyers, and the agent Pro plan for anyone
// who turned out to be investing or working in property.

export async function sendBuyerDay7UpsellEmail({ to, propertyAddress, reportId }) {
  const base = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.reportdecoded.com.au';
  const reportUrl = `${base}/results?reportId=${reportId}`;
  const agentUrl  = `${base}/agents`;
  const addressLine = propertyAddress
    ? `<strong style="color:#0A1628">${propertyAddress}</strong>`
    : 'your recent property';

  return getResend().emails.send({
    from: FROM,
    to,
    subject: 'Thinking about another property? A few options.',
    html: `<!doctype html>
<html><body style="font-family:'DM Sans',Arial,sans-serif;background:#F7F3EE;margin:0;padding:32px 16px;color:#1C1917">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:32px 30px;border:1px solid #E5E0D8">
    ...
    <p style="font-size:14px;line-height:1.65;color:#374151;margin:0 0 16px">
      Hope the ${addressLine} situation worked out — whether you proceeded, negotiated, or walked away.
      Your report is still live if you need to revisit it:
      <a href="${reportUrl}" style="color:#C97A3A;font-weight:600">view your report →</a>
    </p>

    <p style="font-size:14px;line-height:1.65;color:#374151;margin:0 0 20px">
      If you're continuing to search, a couple of options:
    </p>

    <!-- Option 1: 3-pack -->
    <div style="background:#FAF7F1;border:1px solid #E5E0D8;border-radius:10px;padding:16px 18px;margin:0 0 12px">
      <div style="font-weight:600;font-size:14px;color:#0A1628;margin-bottom:6px">
        Looking at multiple properties? 3-Report Pack — $149
      </div>
      <div style="font-size:13.5px;line-height:1.6;color:#374151">
        Three full analyses for $50 each instead of $39. Ideal if you're evaluating a shortlist before committing to one.
        <br/>
        <a href="${base}/#buyer-upload" style="color:#C97A3A;font-weight:600;margin-top:6px;display:inline-block">Upload another report →</a>
      </div>
    </div>

    <!-- Option 2: agent plan (for investors / agents in disguise) -->
    <div style="background:#FAF7F1;border:1px solid #E5E0D8;border-radius:10px;padding:16px 18px;margin:0 0 20px">
      <div style="font-weight:600;font-size:14px;color:#0A1628;margin-bottom:6px">
        Buyer's agent or investor? From $79/month
      </div>
      <div style="font-size:13.5px;line-height:1.6;color:#374151">
        Unlimited reports, white-label PDF for your clients, and a dashboard to manage everything.
        First report free when you sign up.
        <br/>
        <a href="${agentUrl}" style="color:#C97A3A;font-weight:600;margin-top:6px;display:inline-block">See agent pricing →</a>
      </div>
    </div>

    <p style="font-size:13px;line-height:1.55;color:#6B7280;margin:0">
      Either way — good luck with the search. Reply to this email if you want advice on reading a specific defect.<br/><br/>
      Morgan · Founder, Report Decoded<br/>
      <a href="mailto:info@reportdecoded.com.au" style="color:#C97A3A">info@reportdecoded.com.au</a>
    </p>
  </div>
...
// Day-3 review request. ACCC-clean: the review ask carries NO incentive
// (never conditional on leaving one, or on it being positive). The optional
// thank-you discount is framed as a returning-customer perk that applies
// regardless — the two just share the email. discountCode/discountLabel are
// optional; when absent the discount block is omitted entirely.
export async function sendBuyerReviewRequestEmail({ to, propertyAddress, reportId, discountCode, discountLabel }) {
  const base = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.reportdecoded.com.au';
  const reviewUrl = process.env.NEXT_PUBLIC_REVIEW_URL || 'https://g.page/r/CaaXqJlsDzMaEAE/review';
  const reportUrl = `${base}/results?reportId=${reportId}`;
  const addressLine = propertyAddress
    ? `<strong style="color:#0A1628">${escapeHtml(propertyAddress)}</strong>`
    : 'your report';
  ...
  const discountBlock = discountCode
    ...
    <div style="background:#FAF7F1;border:1px solid #E5E0D8;border-radius:10px;padding:16px 18px;margin:0 0 22px">
      <div style="font-weight:600;font-size:14px;color:#0A1628;margin-bottom:6px">
        A thank-you for next time${discountLabel ? ` — ${escapeHtml(discountLabel)} off your next report` : ''}
      </div>
      <div style="font-size:13.5px;line-height:1.6;color:#374151">
        Whenever you're looking at another property, this code is yours — enter it at checkout:
        <div style="margin-top:10px">
          <span style="display:inline-block;font-family:'DM Mono',monospace;font-size:16px;font-weight:700;letter-spacing:1px;color:#0A1628;background:#fff;border:1px dashed #C97A3A;border-radius:8px;padding:8px 16px">${escapeHtml(discountCode)}</span>
        </div>
        <a href="${base}/#buyer-upload" style="color:#C97A3A;font-weight:600;margin-top:10px;display:inline-block">Decode another report →</a>
      </div>
    </div>`
    : '';
  ...
  return getResend().emails.send({
    from: FROM,
    to,
    subject: 'How did the report work out?',
    html: `<!doctype html>
<html><body style="font-family:'DM Sans',Arial,sans-serif;background:#F7F3EE;margin:0;padding:32px 16px;color:#1C1917">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:32px 30px;border:1px solid #E5E0D8">
    ...
    <p style="font-size:14px;line-height:1.65;color:#374151;margin:0 0 16px">
      Hope decoding ${addressLine} helped you make the call — whether you proceeded, negotiated, or walked away.
      It's still live if you need it: <a href="${reportUrl}" style="color:#C97A3A;font-weight:600">view your report →</a>
    </p>

    <p style="font-size:14px;line-height:1.65;color:#374151;margin:0 0 12px">
      If it saved you time or money, an honest review would genuinely help a small operation like mine get off the ground. Takes about 30 seconds:
    </p>

    <div style="margin:0 0 22px">
      <a href="${reviewUrl}" style="display:inline-block;background:#C97A3A;color:#fff;font-weight:700;font-size:14px;text-decoration:none;border-radius:10px;padding:12px 24px">Leave a review →</a>
    </div>
    ${discountBlock}
    <p style="font-size:13px;line-height:1.55;color:#6B7280;margin:0">
      No worries at all if you'd rather not — either way, thanks for giving it a go. Reply to this email anytime if you want a hand reading a specific defect.<br/><br/>
      Morgan · Founder, Report Decoded<br/>
      <a href="mailto:info@reportdecoded.com.au" style="color:#C97A3A">info@reportdecoded.com.au</a>
    </p>
  </div>
