 ...
 *
 * Excludes status='failed' rows so:
 *   - A wrong-PDF upload that got rejected at the prescreen doesn't
 *     count toward the 12/month Starter allowance
 *   - A Claude / Stripe / runtime hiccup that flips the row to failed
 *     doesn't count either
 *
 * 'processing' rows DO count (in-flight uploads use a slot until they
 * resolve). They convert to either 'complete' (still counted) or
 * 'failed' (drops out of the count).
 */
