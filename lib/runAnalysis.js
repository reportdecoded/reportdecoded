...
async function _runAnalysisInternal(reportId, supabase) {
  ...
  // 5b. Fetch the agent row once — used by both the overage check (5c)
  //     and the branded-PDF generation (6). Only relevant when this report
  //     came from an agent dashboard upload (report.agent_id is set).
  let agentFull = null;
  if (report.agent_id) {
    const { data } = await supabase
      .from('agents')
      .select('id, business_name, logo_url, accent_color, subscription_tier, stripe_customer_id')
