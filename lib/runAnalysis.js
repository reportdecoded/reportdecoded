...
import { getSuburbMarketContext } from './valuation';
...
async function _runAnalysisInternal(reportId, supabase) {
  ...
  if (!result.ok) {
    ...
    // Attempt automatic Stripe refund when the failure is our fault
    // (credit exhaustion, overload, scanned PDF we can't read, etc.).
    // Previously this was a TODO with a lying email — now we actually
    // call Stripe before telling the buyer their money is on its way back.
    let refundIssued = false;
    if (result.refund && report.stripe_session_id) {
      try {
        ...
        if (piId) {
          await stripe.refunds.create({
            payment_intent: piId,
            reason: 'fraudulent', // closest Stripe reason for "we failed to deliver"
            metadata: { reportId, failure_reason: (result.error || '').slice(0, 200) },
          });
          refundIssued = true;
          console.log(`[runAnalysis] refund issued for ${reportId} (pi: ${piId})`);
        } else {
          console.warn(`[runAnalysis] no payment_intent on session ${report.stripe_session_id} — refund skipped`);
        }
      } catch (refundErr) {
        // Stripe refund failed — log loudly so Morgan can action manually.
        // Do NOT email the buyer saying refund was issued if it wasn't.
        console.error(
          `[runAnalysis] ⚠️ STRIPE REFUND FAILED for ${reportId}:`,
    ...
    await supabase
      ...
      .update({
        ...
        ...(refundIssued ? { payment_status: 'refunded' } : {}),
      })
      .eq('id', reportId);

    // Email the buyer — but ONLY say "refund processed" if Stripe confirmed it.
  ...
  // 4b. Suburb market context (best-effort; never blocks completion).
  //     getSuburbMarketContext returns null when DOMAIN_API_KEY is unset or
  //     the address can't be parsed — so this is a no-op until a key exists.
  //     Stored inside result_json (jsonb) under `market_context`, so no DB
  //     migration is needed. Lets /results frame the negotiation amount
  //     against the local median ("at/above/below suburb norm").
  try {
    const marketContext = await getSuburbMarketContext({ address: analysis.property_address });
    if (marketContext) {
      analysis.market_context = marketContext;
      console.log(`[runAnalysis] ${reportId} suburb context: ${marketContext.suburb} ${marketContext.state} median ${marketContext.medianPrice}`);
    }
  } catch (e) {
    console.error('[runAnalysis] suburb market context failed (non-fatal):', e?.message || e);
  }
