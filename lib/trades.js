...
export const TRADES = {
  bricklayer: {
    ...
    strong_keywords: [
      ...
      // NOTE: "out of plumb" is ONLY a bricklayer strong signal when paired
      // with masonry context (brick/veneer/wall tie/mortar). Internal walls
      // bowing due to subfloor settlement (hallway walls, partition walls,
      // whole-house movement from stumps) should route to licensed_builder
      // — see licensed_builder strong_keywords below. (Jun 2026 audit:
      // Bill's 1936 Thornbury hallway wall defect was routing to bricklayer
      // because bare "out of plumb" fired; it's a structural/settlement
      // issue, not a brick veneer tie failure.)
      /\bbrick(?:work)?.{0,30}out of plumb\b/i,
      /\bout of plumb.{0,30}brick(?:work)?\b/i,
      /\bveneer.{0,30}out of plumb\b/i,
      /\bmasonry.{0,30}out of plumb\b/i,
      /\bmissing brickwork\b/i,
      ...
      // Corroding/steel lintels in brickwork (Jun 2026 ground-truth
      // audit — Bentley WA garage). Rust-jacked lintel replacement is
      // masonry work: cut out bricks, swap lintel, rebuild. Carpenter
      // only owns TIMBER lintels (see carpenter strong_keywords).
      /\b(?:steel|corrod\w+|rust\w+) lintels?\b/i,
      /\blintels?.{0,50}brick(?:work)?\b/i,
      /\bbrick(?:work)?.{0,50}lintels?\b/i,
    ],
    keywords: [
  ...
  restumper: {
    ...
    strong_keywords: [
      ...
      // Bouncy/springy floors = subfloor stiffness problem (stumps,
      // bearers, joists). Jun 2026 ground-truth audit: "Bouncy and
      // Creaky Interior Floors" (Bentley WA) routed to pest_controller
      // because the description mentioned on-site termites — but the
      // FIX starts with a subfloor specialist, not a pest sprayer.
      /\b(?:bouncy|springy)\b.{0,40}\bfloors?\b/i,
      /\bfloors? feel (?:springy|bouncy)\b/i,
      /\b(?:deteriorat\w+|rott\w+) (?:stumps?|bearers?|joists?)\b/i,
    ],
    keywords: [
      /\bstump(?:s|ing)?\b/i,
      /\bunderpinning\b/i,
  ...
  carpenter: {
    ...
    strong_keywords: [
      ...
      // Timber lintels only — bare "lintel" was routing CORRODING STEEL
      // lintels in brickwork to carpenter (Bentley WA garage defect,
      // Jun 2026 ground-truth audit). Steel/corroded lintels in masonry
      // are bricklayer work — see bricklayer strong_keywords.
      /\btimber lintel\b/i, /\bbarge board\b/i, /\bnogg?in\b/i,
  ...
  plasterer: {
    ...
    keywords: [
      ...
      /\bceiling crack(?:s|ing|ed)?\b/i, /\bwall crack(?:s|ing|ed)?\b/i,
  ...
  roofer: {
    ...
    strong_keywords: [
      ...
      // Flashing defects — metal flashing install/repair (roof, wall, cap,
      // cladding-to-brick junctions) is roof-plumber work, NOT a bricklayer's.
      // Jun 2026 (Morgan): "Roof Flashings — Missing/Inadequate at Cladding-to-
      // Brick Junctions" routed to Bricklayer because the brick/masonry mentions
      // in the text outscored the weight-1 "flashing" keyword.
      /\b(?:metal|roof|wall|cap|cladding|perimeter) flashing(?:s)?\b/i,
      /\bflashing(?:s)?\b.{0,30}(?:missing|inadequate|absent|incomplete|defective|damaged|cracked|corroded|rusted|failed|required|gaps?)\b/i,
      /\b(?:missing|inadequate|no|defective|damaged)\b.{0,20}\bflashing/i,
      ...
      /\bshed roof\b/i,
      // Gutter defect phrasings — moved from keywords to strong so that
      // water-consequence language (water ingress, damp, mould) in the
      // why-it-matters block doesn't pull waterproofer above roofer.
      // The FIX for a blocked/broken/overflowing gutter is always a
      // roofer / roof plumber, not a waterproofer. (Jun 2026 audit.)
      /\bgutter(?:s|ing)? (?:blocked|clogged|overflowing|overflow|broken|cracked|rusted|corroded|leaking|sagging|damaged|missing|replaced?|repaired?|cleaned?|reinstated?)\b/i,
      /\bblocked (?:gutters?|downpipes?)\b/i,
      /\boverflowing (?:gutters?|downpipes?)\b/i,
      /\bdownpipe(?:s)? (?:blocked|broken|cracked|damaged|missing|disconnected|replaced?)\b/i,
      /\bgutter (?:fall|grade|slope|pitch|bracket|guard|leaf|mesh|seal)\b/i,
      /\bgutters? (?:not |in)?adequately (?:falling|pitched|graded|sloped)\b/i,
      /\brainwater head\b/i,
      /\bstormwater (?:outlet|connection|pipe|downpipe)\b/i,
    ],
    keywords: [
      /\broof\b/i,
      /\bridge\b/i, /\bvalley\b/i, /\bapex\b/i,
      /\bflashing\b/i,
      /\bgutter(?:s|ing)?\b/i, /\bdownpipe\b/i, /\beaves gutter\b/i,
  ...
  plumber: {
    ...
    strong_keywords: [
      ...
      // Downpipes discharging to ground / soakwell & stormwater
      // connection work — licensed-plumber territory in AU. Jun 2026
      // ground-truth audit: Bentley WA defect routed to waterproofer
      // because the prose mentioned rising damp / subfloor moisture
      // (the CONSEQUENCE) rather than the fix (stormwater connection).
      /\bdownpipes? (?:discharg|dump)\w*/i,
      /\bsoakwells?\b/i,
      /\bstormwater (?:system|connection|drain(?:s|age)?)\b/i,
      /\bconnect\w* to (?:a )?(?:soakwell|stormwater)/i,
    ],
    keywords: [
  ...
  handyman: {
    ...
    strong_keywords: [
      ...
      /\bweather ?seals?\b/i,   // "Door — Missing Weather Seals" (re-fit seal = handyman)
      // Missing/displaced ceiling manhole cover — fitting a new hatch is
      // a 30-minute handyman job. Jun 2026 ground-truth audit: routed
      // to pest_controller because the why-it-matters prose mentioned
      // "pests, vermin" getting in through the hole.
      /\bmanhole covers?\b/i, /\bmissing manhole\b/i,
      // Window/door perimeter sealant & weather-seal re-caulking — minor
      // maintenance (re-run the silicone bead / replace the weather seal).
      // Jun 2026 audit: "Window Sealant — Deteriorated" was returning
      // NO_TRADE because no trade carried a sealant/caulking pattern.
      // Scoped to window/door so it doesn't poach wet-area sealant
      // defects (those belong to waterproofer/tiler).
      /\bwindow sealant\b/i,
      /\b(?:window|door) (?:perimeter )?(?:caulk(?:ing)?|sealant|weather ?seal)s?\b/i,
      /\b(?:perished|deteriorated?|cracked|missing|failed|gaps? in) (?:window |door )?(?:caulk(?:ing)?|sealant|weather ?seal)\b/i,
  ...
  landscaper: {
    ...
    strong_keywords: [
      ...
      // Paving defect phrasings — landscaper lifts, re-lays, or
      // replaces paving. "Cracked/damaged/broken front paving" is
      // landscaper work, not concreter (Jun 2026 audit: bare "paving"
      // keyword weight=1 was losing to concreter when crack/damage
      // language appeared in the defect description).
      /\b(?:cracked?|damaged|broken|deteriorated?|displaced?|heaved|sunken?|settled?|uneven|lifted|lifting|loose|missing) (?:front |rear |side |driveway |path )?paving\b/i,
      /\bpaving (?:cracked?|damaged|broken|deteriorated?|displaced?|heaved|sunken?|settled?|uneven|lifted|lifting|loose|missing|replaced?|repaired?)\b/i,
      /\bcracked (?:pavers?|paving|path|driveway)\b/i,
      /\bdamaged (?:pavers?|paving|path|driveway)\b/i,
      /\b(?:front|rear|side|entry|driveway) (?:paving|pavers?|path) (?:crack|damage|broken|deterior|displac|heav|sunken|settle|uneven|lift)\b/i,
  ...
  licensed_builder: {
    ...
    strong_keywords: [
      ...
      // Internal wall / whole-house movement — bowing hallway walls,
      // leaning partition walls, walls out of plumb due to subfloor
      // settlement or foundation movement. These are structural assessment
      // jobs (licensed builder first, then restumper/engineer depending on
      // cause), NOT bricklayer jobs. (Jun 2026: Bill's 1936 Thornbury
      // "Bowing/Leaning Hallway Walls 50mm out of plumb" was routing to
      // bricklayer; fix moves bare "out of plumb" here.)
      // Hallway / internal wall movement — allow gaps between "walls" and
      // movement verb since Claude writes "walls are leaning" not "walls leaning"
      /\b(?:hallway|internal|partition) walls?.{0,20}(?:bowing|leaning|out of plumb|out of vertical|movement|subsid|settling)\b/i,
      /\b(?:bowing|leaning|out of plumb|bulging).{0,20}(?:hallway|internal|partition) walls?\b/i,
      /\bwalls?.{0,10}(?:bowing|leaning|out of plumb|bulging).{0,60}(?:foundation|subfloor|stump|settlement|floor|roof)\b/i,
      // "pressures pushing...from the roof/floors/foundations" — Claude
      // doesn't always use the contiguous "pressure from" phrase
      /\bpressure[s]?.{0,30}from.{0,10}(?:roof|floor|foundation)/i,
      /\bfrom (?:the )?(?:roof|floor|foundation)[s]? pushing\b/i,
      /\bstructural (?:movement|settlement|failure|assessment|concern|instability)\b/i,
      /\bongoing foundation (?:issues?|movement|problems?|concern)\b/i,
      /\b(?:hallway|internal).{0,30}out of plumb\b/i,
      /\bout of plumb.{0,30}(?:hallway|internal|partition)\b/i,
    ],
    keywords: [
...
// Canonical ordered list of trade keys — single source for the Claude
// trade-assignment enum (see claude.js TRADE_GUIDE) and for validating
// Claude's pick at render/lookup time.
export const TRADE_KEYS = Object.keys(TRADES);

/**
 * Resolve a Claude-assigned trade key to a render/lookup-ready trade object
 * ({key, label, here_queries}). Returns null for missing/unknown keys so
 * callers fall back to regex inference (topTradesForDefect).
 */
export function tradeByKey(key) {
  if (!key || typeof key !== 'string') return null;
  const def = TRADES[key.trim()];
  if (!def) return null;
  return { key: key.trim(), label: def.label, here_queries: def.here_queries };
}
...
const EXPLICIT_TRADE_MENTIONS = [
  ...
  { phrase: 'registered builder', key: 'licensed_builder' }, // AU terminology
  { phrase: 'builder', key: 'licensed_builder' },            // "a builder should assess"
 ...
 * @returns {Array}
 */
// High-precision business-name signatures per trade. A HERE result only shows
// under a trade if its NAME genuinely identifies that trade. Deliberately
// TIGHTER than name_match (which is loose enough to let "Steel Frame & Trusses"
// match carpenter, or "...Construction Academy" match builder). When a name
// doesn't strictly match, we drop it and let the Google Maps fallback handle
// it — better an honest "search Google" than a wrong specialist.
const STRICT_NAME_MATCH = {
  bricklayer: /\bbricklay|\bbrickwork|\bmason(?:ry|s)?\b|\brepointing\b/i,
  concreter: /\bconcret/i,
  restumper: /\brestump|\bunderpin|\breblock|\bhouse (?:level|rais)|floor level/i,
  carpenter: /\bcarpent|\bcarpentry\b|\bjoiner(?:y)?\b|\bchippy\b/i,
  stair_specialist: /\bstair|\bstaircase|\bbalustrad/i,
  plasterer: /\bplaster|\bgyprock|\bcornic|\bdrywall/i,
  tiler: /\btiler|\btiling|\bre-?grout/i,
  flooring_contractor: /\bflooring\b|\bfloor layer|\bcarpet\b|\bvinyl floor|\blaminate floor|\bfloating floor/i,
  painter: /\bpaint(?:er|ing|ers)\b|\bdecorator/i,
  roofer: /\broof(?:er|ing|s)?\b|\broof plumb|\bgutter/i,
  plumber: /\bplumb(?:er|ing|ers)\b|\bdrain(?:age|s|er)|\bgasfit|\bgas fit/i,
  electrician: /\belectric(?:al|ian|ians)\b|\bsparky\b/i,
  glazier: /\bglaz(?:ier|ing)|\bglass\b|\bshower screen/i,
  waterproofer: /\bwaterproof|\bdamp\b/i,
  door_specialist: /\bdoor/i,
  handyman: /\bhandyman|\bmaintenance|\bproperty (?:services|maintenance)|\bhome (?:repair|services)|\ball trades|\bodd job|\bmulti[- ]?trade/i,
  locksmith: /\blocksmith|\bkey (?:cutting|service)/i,
  cabinetmaker: /\bcabinet|\bcabinetry|\bjoiner(?:y)?\b|\bkitchen/i,
  benchtop_specialist: /\bbenchtop|\bstone bench|\bcaesarstone|\bquantum quartz|\bstonemason|\bstone fabricat/i,
  pest_controller: /\bpest|\btermite|\bfumigat|\bvermin/i,
  asbestos_remover: /\basbestos|\bhazmat|\bhazardous material/i,
  insulation_contractor: /\binsulat|\bbatts?\b/i,
  fencer: /\bfenc(?:e|es|ing|er)\b|\bgates? and fences/i,
  garage_door_specialist: /\bgarage door|\broller door|\bpanel ?lift|\bsectional door/i,
  metalworker: /\bmetal ?work|\bsteel\b|\bweld|\bfabricat|\bironwork/i,
  hvac: /\bair ?con|\bhvac\b|\bheating|\bcooling|\brefriger|\bclimate control/i,
  pool_specialist: /\bpool\b|\bspa\b/i,
  renderer: /\brender(?:er|ing)?\b|\bbagging\b/i,
  landscaper: /\blandscap|\bpaving|\bgarden(?:ing|er)|\bturf|\birrigat|\bretaining wall/i,
  licensed_builder: /\bbuild(?:er|ers|ing)\b|\bconstruction|\bconstruct\b|\brenovat|\bcontractor/i,
};

// Generalist tradies (handyman/renovations/general builder) only count for
// builder/handyman-type defects — NOT for specific specialist trades, where a
// generic "X Renovations" isn't the right call.
const GENERALIST_OK_TRADES = new Set(['licensed_builder', 'handyman']);

/** True if a business name strictly identifies the given trade key. */
export function strictTradieMatch(name, tradeKey) {
  const rx = STRICT_NAME_MATCH[tradeKey];
  return !!(rx && name && rx.test(name));
}

export function filterTradiesByInferredTrades(tradies, trades) {
  if (!Array.isArray(tradies) || tradies.length === 0) return [];
  if (!Array.isArray(trades) || trades.length === 0) return [];
  const keys = trades.map((t) => t.key).filter(Boolean);
  if (keys.length === 0) return [];
  const generalistAllowed = keys.some((k) => GENERALIST_OK_TRADES.has(k));
  return tradies.filter((tradie) => {
    const name = (tradie?.business_name || tradie?.name || '').toString();
    if (!name) return false;
    // STRICT: the name must genuinely identify one of the inferred trades.
    if (keys.some((k) => strictTradieMatch(name, k))) return true;
    // Generalists pass only for builder/handyman defects.
    if (generalistAllowed && isGeneralistTradie(tradie)) return true;
    return false;
  });
}
