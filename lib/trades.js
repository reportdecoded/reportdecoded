...
export const TRADES = {
  ...
  door_specialist: {
    ...
    strong_keywords: [
      // Plural-tolerant. The bare /door hinge/ regex was failing on
      // "door hinges" (the \b at end fails when an s follows) — which
      // was the exact phrasing in the Kilmore squeaky-hinge defect
      // that routed to plumber instead.
      /\bdoor (?:hinges?|closers?|seals?|sweeps?|alignments?|tracks?|frames?|adjustments?)\b/i,
      /\bweatherstrip\b/i, /\bweather seal\b/i,
      /\bmisaligned door\b/i, /\bsticking door\b/i, /\bdoor (?:won't|wont) close\b/i,
      // Door type / defect patterns for the most common handover-PCI
      // door defects: hung/adjusted incorrectly, gaps at top, screen
      // and sliding doors, hinge squeaks, realign requests.
      /\b(?:screen|sliding|hinged|french|bi[- ]?fold|cavity|external|entry|internal) door\b/i,
      // No alignment/adjustment/track/frame/seals here — those live in
      // the door-X alternation above. This one captures the remaining
      // door defect verbs.
      /\bdoor (?:misaligned|misalignment|hung)\b/i,
      /\b(?:re)?align(?:ed|ing|s)? (?:the )?(?:door|frame)\b/i,
      /\bhinge (?:squeak|alignment|adjustment|replacement)\b/i,
      /\bsqueak(?:y|s|ing)? (?:hinge|door)/i,
      /\bdoor (?:hung|adjusted) (?:in)?correctly\b/i,
      /\bgap (?:at the top|at the bottom|around the door|in the door frame)\b/i,
    ],
    keywords: [
