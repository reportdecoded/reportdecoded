...
//   - DOMAIN_API_KEY is unset (feature simply doesn't appear)
//   - the address can't be parsed into state + suburb (+ postcode)
//   - the Domain call fails for any reason
// so runAnalysis can always call us and a missing key just means no panel.
//
// ⚠️ The Domain response shape below is parsed defensively against the
//    documented schema but has NOT been verified against a live key yet.
//    Once DOMAIN_API_KEY exists, confirm the JSON path in mapSeries().
