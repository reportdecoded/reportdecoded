            ...
            <>
              ...
              {complianceInspections.map((c, i) => (
                <View key={`cir-${i}`} style={styles.defectCard} wrap={false}>
                  {/* CRITICAL: defectName carries flex:1 (designed for use inside defectHeader's row flexbox). Used standalone in a column-flex card, flex:1 makes the heading expand to fill the entire card height — subsequent elements then render on top of it. Explicitly override flex back to 0. */}
                  <Text style={[styles.defectName, { marginBottom: 4, flex: 0 }]}>{c.type}</Text>
                  {c.why_needed ? (
