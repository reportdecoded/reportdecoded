...
CLASSIFICATION CALIBRATION (apply judgement — do NOT over-classify):
The AS4349.1 "further deterioration" wording, read literally, would sweep almost any maintenance item into "major" — paint, sealant, grout and fences all deteriorate if ignored. Do not do this. A defect belongs in major_defects ONLY when it is genuinely significant, i.e. one of:
- structural (footings, framing, slab, roof structure, significant sagging or movement),
- a safety hazard (electrical, trip/fall on a primary access path, unsafe balustrade, asbestos disturbance),
- active or high-risk moisture / water ingress / drainage affecting the building fabric,
- timber pest activity, or fungal/wood decay to a structural or building element,
- or a genuinely costly / urgent rectification.
Cosmetic and routine maintenance items are MINOR even though they will slowly deteriorate — e.g. repainting, unpainted or weathered timber, re-sealing or re-grouting to wet areas, sealant replacement, expansion-joint sealing, minor gaps to be sealed, leaning or ground-contact timber FENCING and gates, worn hardware, missing or broken drain covers, minor surface unevenness. Put these in minor_defects.
HARD COST RULE (the decisive test): a defect whose estimated repair cost is under ~$1,000 is MINOR by default. It only escapes to major_defects if it is a genuine SAFETY hazard, ACTIVE water ingress / structural movement, or LIVE timber-pest activity. So a ~$200 sealant or grout repair, a ~$350 drain cover, a ~$400 gutter-joint repair, or a ~$600 paint/timber item is NEVER a major defect — regardless of the inspector's wording, its "urgency" rating, or theoretical "further deterioration". Apply this rule to every candidate before you finalise the major list.
SANITY CHECK before finalising: an established home in average condition usually has only a HANDFUL of true major defects (often 2–6), not fifteen-plus. If your major_defects list is long (roughly 8+), re-read it and demote every low-cost, non-urgent, cosmetic or maintenance item to minor_defects. Accurate severity is more credible — and more persuasive in a negotiation — than an inflated major count. Never drop a genuine finding; only classify it in the correct bucket.
...
- SEVERITY CALIBRATION: the inspector has already scoped the defect with their severity language — use it to pick WHERE in the benchmark range you land. "minor", "isolated", "localised", "monitor", "early signs" → bottom half of the range. "moderate", "several", "sections of" → middle. "significant", "extensive", "widespread", "throughout", "advanced", "urgent" → top half. Never output the full benchmark range when the inspector's own words narrow it.

- REGIONAL ADJUSTMENT: adjust both low and high for the property's location (from the address):
  · Sydney metro: +15%
  · Melbourne / Canberra metro: +10%
  · Brisbane / Perth / Adelaide / Hobart / Darwin metro: benchmark as-is
  · Regional cities and towns (e.g. Geelong, Ballarat, Newcastle, Toowoomba): −10%
  · Remote / far-regional (limited local trades): +20% for travel and availability
  Round adjusted figures to the nearest $50 (under $1,000) or $100 (above).

- ACCESS & CONTEXT MULTIPLIERS: real quotes blow out on access, not materials. When the report implies any of these, add the premium and name it in plain_english:
  · Two-storey roof, gutter, eave or external wall work → scaffolding/EWP hire: add $1,500-$4,000
  · Wet-area demolition or sheet removal in a pre-1990 home → asbestos testing + licensed removal/disposal: add 30-100% to that defect
  · Heritage overlay or period features explicitly mentioned → add 20% and note possible council approval requirements
  · Subfloor access described as restricted/limited/no access → add 20% to subfloor work
  · Steep site, rear-of-block or no vehicle access mentioned → add 10-15%

- SAME-TRADE BUNDLING: a buyer does not pay ten call-out fees for ten small jobs. When 3+ defects share the same trade and each is individually small (midpoint under ~$500), price them as ONE bundled visit: the first item carries the call-out, the rest are marginal labour (e.g. a painter's day at $600-$900 covers many touch-up items; a handyman day knocks out 6-10 small fixes). Reflect this in the per-defect ranges so total_repair_cost is not inflated by stacked minimum charges. Note the bundling in the summary so the buyer understands the totals assume consolidated visits.

- ERA & MATERIALS: use the building era and named materials to sharpen the range on their own — e.g. terracotta vs Colorbond re-roof are different benchmark lines; pre-1990 wet areas carry the asbestos premium above; homes on stumps imply restumper-scale subfloor work where slab homes imply concrete repair. When the report names the material, never price the generic alternative.

- COST CONFIDENCE: set cost_confidence per defect. "high" = the report states quantity or clear extent AND a specific benchmark line fits. "medium" = benchmark fits but extent is partly inferred. "low" = vague extent, concealed/inaccessible area, or no close benchmark. The range width should reflect this: high-confidence ranges are tight; low-confidence ranges honestly wide.

- ASSESSMENT-ONLY DEFECTS: some defects cannot be honestly priced from a visual report — underpinning/subsidence extent, concealed termite or structural damage, anything where the inspector says scope is unknowable without invasive or specialist investigation. For these set cost_basis = "assessment_only" and make repair_cost_low/high the cost of the SPECIALIST ASSESSMENT (structural engineer $1,500-$3,500; invasive timber pest inspection $400-$800; CCTV drain survey $300-$600). Say clearly in plain_english that the true repair cost is unknowable until that assessment is done — a guessed range on these defects is misleading, and the buyer must get the assessment BEFORE exchange. For all other defects set cost_basis = "benchmark". An assessment_only defect must NEVER have repair_cost_low/high of 0 — when one specialist visit covers several related defects (e.g. an engineer assessing both a leaning pier and unlevel floors), put the full assessment cost on the most significant defect and a nominal share (minimum $100) on the others, noting the shared visit in plain_english.

- LIKELY TOTAL: set total_repair_cost_likely = the sum of every defect's range midpoint, rounded to the nearest $500. This is the headline number a buyer should plan around: independent ranges don't all land at their maximum simultaneously, so the low-to-high spread overstates real-world uncertainty. total_repair_cost_low/high remain the honest bounds. Exclude assessment_only defects' hypothetical repairs from all three totals — their assessment cost is what's included.
...
Step 1: base_midpoint = total_repair_cost_likely (the sum of per-defect range
        midpoints you computed above — keeps the headline total and the
        negotiation ask telling the same story)
  ...
  "major_defects": [
    {
      ...
      "repair_cost_low": 0,
      "repair_cost_high": 0,
      "cost_confidence": "high | medium | low — high when the report states quantity/extent and a specific benchmark line fits; low when extent is vague or the area was concealed/inaccessible",
      "cost_basis": "benchmark | assessment_only — assessment_only when the true repair scope is unknowable without specialist investigation (underpinning extent, concealed termite damage, structural movement). For assessment_only, repair_cost_low/high must be the cost of the ASSESSMENT (e.g. structural engineer $1,500-$3,500), NOT a guessed repair range",
      "trade_category": "roofing | plumbing | building | pest | electrical | damp",
      "trade": "PRIMARY trade key from TRADE ASSIGNMENT (e.g. painter, roofer, plumber, handyman) — performs the main fix",
      "trade_secondary": "SECOND trade key — a DIFFERENT, next-most-relevant trade (always provide one; see TRADE ASSIGNMENT)",
...
const TRADE_GUIDE = `TRADE ASSIGNMENT — for EVERY defect (major, minor, AND pest finding), set the "trade" field to the SINGLE trade key below that performs the PRIMARY remedy. Choose by the FIX REQUIRED, not by the material or the location it sits in. Worked examples of the distinction:
- a TIMBER door whose edges need sealing/painting → painter (NOT carpenter — the fix is paint, not carpentry)
- metal flashing missing/inadequate, even at a brick or cladding junction → roofer (NOT bricklayer — flashing is roof-plumber work)
- door weather seals missing, re-caulking, hinge/adjustment, minor re-fit → handyman
Ask yourself: which ONE tradesperson would the buyer actually phone to fix this? Output that key.

Use the key EXACTLY as written:
- bricklayer — brickwork, mortar, repointing, brick piers, masonry cracking
- concreter — concrete slabs, footings, spalling, concrete cancer
- restumper — restumping/underpinning, subfloor stumps, sagging or unlevel floors
- carpenter — structural timber framing, joists/bearers/studs, decking rebuild, replacing rotten timber, re-fixing cladding
- stair_specialist — stairs, treads, risers, balustrades, handrails
- plasterer — plasterboard, cornices, wall/ceiling cracks and patching
- tiler — wall/floor tiling, grout, regrouting, cracked or loose tiles
- flooring_contractor — laminate, vinyl, hybrid, carpet, floating floors
- painter — painting, sealing/coating/varnishing bare or unsealed timber, paint touch-ups, unpainted edges
- roofer — roof coverings, tiles, ridge capping, FLASHINGS, gutters, downpipes, sarking
- plumber — pipes, drains, taps, hot water, leaks under fixtures, gas fitting
- electrician — wiring, switchboard, RCD/safety switch, smoke alarms, lighting, exhaust fans
- glazier — glass, windows, shower screens, mirrors
- waterproofer — waterproofing membranes, rising damp, wet-area/balcony/shower waterproofing failure
- door_specialist — NEW door installs, cavity sliders, security doors (NOT minor door adjustments)
- handyman — minor maintenance: hinges, weather seals, re-caulking, small adjustments, sliding/robe doors, loose handles, builder cleanup, curtain/blind cords
- locksmith — locks, deadlocks, rekeying, keys
- cabinetmaker — kitchen/bathroom cabinetry, joinery, cupboards, vanities, drawers
- benchtop_specialist — stone or laminate benchtops (cracked/chipped/repair)
- pest_controller — termite/borer/pest damage, treatment, termite management systems
- asbestos_remover — asbestos / fibro removal or assessment
- insulation_contractor — ceiling/wall/underfloor insulation, batts, R-value
- fencer — fences, gates, paling/Colorbond, pool barriers
- garage_door_specialist — garage/roller/panel-lift doors and motors
- metalworker — structural steel, metal balustrades/posts/brackets, welding
- hvac — air conditioning, heating, ducted/split systems
- pool_specialist — pools, spas, pool equipment
- renderer — cement/acrylic render, bagging
- landscaper — paving, retaining walls, site drainage/grading, gardens, ground falls toward the house
- licensed_builder — multi-trade or structural defects, unknown-cause investigations, significant structural concerns

If genuinely none fit, use licensed_builder. The "trade_category" field stays as the broad bucket; "trade" is the specific, authoritative pick.

TWO TRADES PER DEFECT (required): always provide BOTH "trade" (primary — does the main fix) AND "trade_secondary" (a DIFFERENT, next-most-relevant trade the buyer may also need). The two must be different keys. Natural pairings:
- rotten/decayed painted timber → carpenter (replace timber) + painter (re-seal/repaint)
- termite-damaged framing → pest_controller (treat) + carpenter (repair)
- shower/wet-area leak → waterproofer (re-membrane) + tiler (re-tile)
- rising damp → waterproofer + plasterer (re-render)
- leaning pier / subsidence → restumper (or bricklayer) + licensed_builder (structural assessment)
- cracked render over brick → renderer + bricklayer
- failed flashing causing internal damage → roofer (flashing) + plasterer (repair water-damaged lining)
If a second trade is genuinely a stretch, choose the closest complementary trade (often handyman or licensed_builder), but always populate both.

SELF-CHECK before finalising: re-read each defect's "trade" and "trade_secondary" and confirm each is the trade that performs the ACTUAL FIX, not the material or location. Common mistakes to catch and correct: a paint/seal job on timber assigned to carpenter (should be painter); flashing assigned to bricklayer because the defect text mentions brick (should be roofer); a minor seal/adjustment assigned to licensed_builder (should be handyman); a specific trade defaulted to licensed_builder when a precise trade clearly applies. Only use licensed_builder when the defect genuinely needs multiple trades or a structural investigation.`;
