 ...
 * >}
 */
export async function analyseInspectionPdf({ reportUrl, purchasePrice, reportType = 'pre_purchase' }) {
  if (!reportUrl) {
  ...
  try {
    message = await getClient().messages.create({
      ...
      max_tokens: 8000,
      system: [
        { type: 'text', text: buildSystemPrompt(reportType), cache_control: { type: 'ephemeral' } },
      ],
      messages: [
        {
          role: 'user',
